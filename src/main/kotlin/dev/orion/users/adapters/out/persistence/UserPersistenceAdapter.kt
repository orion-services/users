 ...
 */
@ApplicationScoped
class UserPersistenceAdapter @Inject constructor(
    private val roleRepository: RoleRepository,
    private val mapper: UserEntityMapper
) : PanacheRepository<UserEntity>, UserPersistencePort {
    ...
    override suspend fun authenticate(email: String, passwordHash: String): User? {
    ...
    override suspend fun updateEmail(email: String, newEmail: String): User {
        val result = updateEmailEntity(email, newEmail).awaitSuspending()
        return mapper.toDomain(result)
    }

    override suspend fun validateEmail(email: String, code: String): User {
        val result = validateEmailEntity(email, code).awaitSuspending()
        return mapper.toDomain(result)
    }

    override suspend fun changePassword(password: String, newPassword: String, email: String): User {
    ...
    override suspend fun recoverPassword(email: String): String {
        return recoverPasswordEntity(email).awaitSuspending()
    }
    ...
    override suspend fun updateUser(user: User): User {
        ...
        if (id != null) {
            val entity = findById(id)
                .onItem().ifNull().failWith(IllegalArgumentException("User not found"))
                .awaitSuspending()!!
    ...
    private fun applyDomainScalarsToEntity(domain: User, entity: UserEntity) {
    ...
    private fun authenticateEntity(user: UserEntity): Uni<UserEntity?> {
        return find("email = :email and password = :password", mapOf(EMAIL to user.email, PASSWORD to user.password))
            .firstResult<UserEntity>()
    }

    private fun updateEmailEntity(email: String, newEmail: String): Uni<UserEntity> {
            ...
            .transformToUni { user ->
                ...
                checkEmail(newEmail)
                    ...
                    .switchTo {
                        ...
                        u.emailValid = false
                        u.email = newEmail
                        Panache.withTransaction { u.persist() }
                            .onItem().transform { u }
                    }
            }
    }

    private fun validateEmailEntity(email: String, code: String): Uni<UserEntity> {
        return find("email = :email and emailValidationCode = :code", mapOf(EMAIL to email, "code" to code))
            .firstResult<UserEntity>()
    ...
    private fun changePasswordEntity(password: String, newPassword: String, email: String): Uni<UserEntity> {
        return checkEmail(email)
            .onItem().ifNull()
            .failWith(IllegalArgumentException(USER_NOT_FOUND_ERROR))
            .onItem().ifNotNull()
    ...
    private fun findUserEntityByEmail(email: String): Uni<UserEntity?> {
        return find(EMAIL, email).firstResult<UserEntity>()
    }

    private fun listAllEntities(): Uni<List<UserEntity>> {
        return listAll()
    }
    ...
    private fun checkName(name: String): Uni<UserEntity?> {
        return find("name", name).firstResult<UserEntity>()
    }

    private fun checkHash(hash: String): Uni<UserEntity?> {
        return find("hash", hash).firstResult<UserEntity>()
    }
